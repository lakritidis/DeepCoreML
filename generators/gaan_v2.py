...
class GMMComponent:
    """A component of a Gaussian Mixture that derives from EM

    By definition, this is either a Multivariate Normal (MVN), or a Normal Distribution that governs the data
    ...
    def create_distribution(self, comp, means, covariances, covariance_type):
        """Create the probability distribution of the cluster data samples with respect to `covariance_type`.

        Parameters
        ----------
        comp : array-like of shape (n_samples, n_dimensions)
            List of n_features-dimensional data points. Each row
            corresponds to a single data point.
        means : array-like of shape (num_components, num_features)
...
class GAANv2(BaseGAN):
    ...
    def create_latent_spaces(self, x_train, y_train):
        ...
        self._probability_matrix = np.zeros((self._n_classes, self._num_gmm_components))
        for comp in range(self._num_gmm_components):
            x_comp = x_train[cluster_labels == comp, :]
            y_comp = y_train[cluster_labels == comp]

            cluster = GMMComponent(comp, x_comp, y_comp, self._random_state)
        ...
        for c in range(self._n_classes):
            ...
            for comp in range(self._num_gmm_components):
                cluster = self._gmm_components[comp]
        ...
        x_transformed = self._gmm_components[0].fit_transform()
        for comp in range(1, self._num_gmm_components):
            x_transformed = np.concatenate((x_transformed, self._gmm_components[comp].fit_transform()))
    ...
    def sample_latent_space(self, num_samples):
        ...
        for s in range(num_samples):
            ...
            lat_cluster = np.random.choice(a=np.arange(self._num_gmm_components, dtype=int), size=1,
                                           replace=True, p=self._probability_matrix[lat_class])

            latent_cluster_object = self._gmm_components[int(lat_cluster)]
            ...
            z = latent_cluster_object.get_prob_distribution().sample()
            latent_x.append(z)
    ...
    def generator_loss(self, predicted_labels, real_labels):
    ...
    def train_batch(self, real_data):
        ...
        gen_loss = self.generator_loss(d_predictions, real_labels.to(self._device))
        gen_loss.backward()
